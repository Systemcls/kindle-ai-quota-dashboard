...
发布通过 GitHub 官方接口创建网页提交并更新 `gh-pages`，使用电脑现有 GitHub 登录，凭据仅在内存中使用；不会强制推送。锁文件位于被忽略的 `state/` 内。若前一次发布进程异常退出留下锁文件，确认没有发布进程运行后，删除 `state/pages-publish.lock` 再重试。

若 GitHub 直连不稳定，可在 `.env` 中设置 `GITHUB_PROXY` 为电脑已运行的 HTTP 代理地址。代理模式使用系统 curl，认证通过标准输入传入，不写入文件或命令参数；不会修改全局代理或关闭证书验证。
